...
namespace Client.Pages
{
    public partial class Technologies
    {
        ...
        private async Task CacheDataAsync()
        {
            ...
            var categoryResult = await AddItemsToIndexedDbAsync(Constants.CategoriesObjectStoreName, TechnologyCategoryList);
        }
        ...
        private async Task<bool> FillListsFromCacheAsync()
        {
            ...
            var technologies =
                await GridColumnDataIndexedDb.GetAll<GetTechnologiesQuery>(Constants.TechnologiesObjectStoreName);
            var categories =
                await GridColumnDataIndexedDb.GetAll<GetTechnologyCategoriesQuery>(Constants.CategoriesObjectStoreName);

            if (!technologies.Any() || !categories.Any())
                return false;

            TechnologyList = technologies;
            TechnologyCategoryList = categories;
